...
const HEAD_CLEARANCE = 1.35;
/** Used only when a rider has no neck to find, measured up from the mount's root. */
...
interface Plate {
  ...
  transform: string;
  depth: number;
  /** The rider model the neck was found on, so a body swap looks again. */
  model: Object3D | null;
  neck: Object3D | null;
}

/**
 * The small name chips over every rider's head.
 *
 * DOM, not geometry. A plate is text and a portrait, which the browser already
 * draws crisply at every pixel ratio; a canvas texture per player would have
 * to be re-rendered whenever a name changed and would blur as it shrank. There
 * are at most fifteen of these, one room's worth.
 *
 * Positioned AFTER the frame is rendered, from the matrices that render just
 * computed. Projecting before it would place every plate where its rider was
 * one frame ago - invisible at a walk, and a label trailing metres behind a
 * mount covering four hundred units a second.
 *
 * Every write is guarded by a comparison, and nothing here ever READS layout:
 * the viewport size is handed in on resize rather than measured per frame, so
 * moving fifteen elements costs fifteen transforms and never a forced reflow.
 */
export class Nameplates {
  ...
  constructor(container: HTMLElement) {
    ...
    this.layer.className = 'aoe-plates';
  ...
  /** Place one rider's plate for this frame. */
  put(key: string, mount: Mount, name: string, pfp: string): void {
    ...
    // Nearer riders draw over farther ones, as they would in the world.
  ...
  private create(key: string): Plate {
    ...
    root.className = 'aoe-plate aoe-plate--bare';
    root.hidden = true;

    const pfp = document.createElement('img');
    pfp.className = 'aoe-plate__pfp';
    pfp.alt = '';
    pfp.draggable = false;
    pfp.decoding = 'async';
    pfp.hidden = true;
    ...
    pfp.addEventListener('error', () => {
      pfp.hidden = true;
      root.classList.add('aoe-plate--bare');
    });

    const label = document.createElement('span');
    label.className = 'aoe-plate__name';
  ...
  private write(plate: Plate, name: string, pfp: string): void {
    ...
    if (plate.src !== pfp) {
      ...
      plate.root.classList.toggle('aoe-plate--bare', !pfp);
    }
  }

  /**
   * Where over a rider the plate goes, in world space.
   *
   * Over the NECK rather than a fixed height above the mount, so it follows
   * the rider's own proportions, every animal's different saddle height and a
   * body swap alike. The neck is looked up again whenever the rider model
   * changes - a Bloxity body arriving replaces every bone object.
   */
  private anchor(plate: Plate, mount: Mount, out: Vector3): void {
    const model = mount.rider.model;
    ...
    if (plate.neck) {
      ...
      out.y += HEAD_CLEARANCE * mount.rider.visual.scale.y;
    } else {
      mount.root.getWorldPosition(out);
