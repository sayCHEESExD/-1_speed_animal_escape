 ...
 * animal is affordable, whether the player is really standing there - is made
 ...
 * the server's confirmation means the animal keeps galloping through thin air
...
export class RunController {
  ...
  setInventory(ownedAnimals: number, wins: number): void {
  ...
  update(delta: number, player: LocalPlayer, elapsed: number): void {
    ...
    this.animalCooldown = Math.max(0, this.animalCooldown - delta);
    ...
    if (slot !== null && this.animalCooldown === 0) {
      // Asking for an animal already owned, or one the player plainly cannot
      ...
      if (!ownsAnimal(this.ownedAnimals, slot) && this.wins >= animal.winsRequired) {
        this.animalCooldown = REQUEST_COOLDOWN;
        this.actions.claimAnimal(slot);
      }
    }
  }
