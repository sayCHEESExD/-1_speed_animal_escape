import { ELEPHANT } from '@animal/shared';
import { Group, Mesh, MeshLambertMaterial, type BufferGeometry } from 'three';
import { BoxSet } from '../animal/BoxSet.js';

/** How fast the rendered elephant eases toward its replicated transform. */
const FOLLOW_RATE = 12;

/** Distance past which it is placed rather than eased. */
const SNAP_DISTANCE = 30;

/** Palette. Grey stone-ish, so it belongs among the ruins it lives in. */
const HIDE = 0x9d9aad;
const HIDE_DARK = 0x86839a;
const TUSK = 0xf4efe2;
const PAD = 0x5d5a6b;
 ...
 *
 * Built from the same `BoxSet` the rideable animals use, so it is the same
...
export class Elephant {
  ...
  constructor() {
    ...
    // Ears: big flat plates, the fastest way to say "elephant" in boxes.
    ...
    this.root.position.set(0, ELEPHANT.shoulderY, (ELEPHANT.minZ + ELEPHANT.maxZ) / 2);
  }
  ...
  update(delta: number): void {
    ...
    // The body rocks, the head bobs and the trunk swings - a charging elephant
