 ...
 * animal leaves the ground. A client cannot ask for Speed, and a single step
...
export class SpeedService {
  ...
  credit(sessionId: string, player: PlayerState, stepSeconds: number): SpeedGain {
    ...
    // ONE rate, and it is simply the equipped animal's own figure. There is no
    // arithmetic here on purpose: what an animal is worth belongs in the
    // animal roster, and a second multiplier applied at the point of payment
    // is how two systems end up disagreeing about a player's income.
    const perStride = animalForSlot(player.animalSlot).speedPerStep;
    ...
    if (!tracker.fresh) {
      if (player.treadmill > 0) {
        ...
        gained += (distance / SPEED.strideDistance) * perStride * TRAINING.multiplier;
      } else {
        ...
        // scales with the animal exactly as travel does.
   ...
   *
   * Used on join, on reconnect and whenever the equipped animal changes: the
  ...
  syncDerived(player: PlayerState): void {
    ...
    const animal = animalForSlot(player.animalSlot);
    player.speedPerStep = animal.speedPerStep;
