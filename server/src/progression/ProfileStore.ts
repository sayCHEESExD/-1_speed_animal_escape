import { INITIAL_OWNED_ANIMALS } from '@animal/shared';
...
class ProfileStore {
   ...
   * and the equipped animal are all recomputed by their own services from
  ...
  restore(playerId: string, player: PlayerState): boolean {
    ...
    player.ownedAnimals = profile.ownedAnimals | INITIAL_OWNED_ANIMALS;
  ...
  save(playerId: string, player: PlayerState): void {
    ...
    this.profiles.set(playerId, {
      ...
      wins: player.wins,
      ownedAnimals: player.ownedAnimals,
