...
import {
  ...
  SPAWN_ROTATION_Y,
  STARTER_ANIMAL_SLOT,
  type AnimalAnimationState as AnimationState,
} from '@animal/shared';
 ...
 * the authoritative simulation; progression, Wins and the owned-animal set are
 * written only by their own service. Nothing is ever copied from a client
 * message.
 *
 * Note what is NOT here: bone rotations, animal part transforms, gait timers.
...
export class PlayerState extends Schema {
  ...
  /** Equipped animal slot - the best one owned. Written by AnimalService. */
  @type('uint8') animalSlot = STARTER_ANIMAL_SLOT;
  /** Bitmask of animals claimed. Written by AnimalService only. */
  @type('uint32') ownedAnimals = INITIAL_OWNED_ANIMALS;
  /** Speed granted per stride by the equipped animal. */
  @type('float32') speedPerStep = 1;

  /**
   * Authoritative movement multiplier and jump velocity, resolved from level,
   * rebirth and the equipped animal by the one shared formula. The client
