import { ELEPHANT, MOUNT_RADIUS, inElephantTerritory } from '@animal/shared';
import type { ElephantState } from '../rooms/state/CourseState.js';
...
/**
 * The elephant, simulated on the server.
 *
 ...
 * authoritative positions it already has, moves the elephant, and decides the
 * kill from its own numbers. The client receives x, z, yaw and a charging flag
 * and does nothing but draw them.
 *
 * There is no elephant message. A client cannot move it, cannot claim to have
...
export class ElephantService {
  ...
  reset(state: ElephantState): void {
    state.x = 0;
    state.z = (ELEPHANT.minZ + ELEPHANT.maxZ) / 2;
   ...
   */
  update(state: ElephantState, delta: number, players: Iterable<PlayerState>): void {
    ...
    if (prey) {
      ...
      goalX = 0;
      goalZ = this.patrolDirection > 0 ? ELEPHANT.maxZ : ELEPHANT.minZ;
    ...
    if (distance > 0.01) {
      ...
      const step = ELEPHANT.turnSpeed * dt * (charging ? 1.5 : 1);
      state.rotationY += Math.abs(turn) <= step ? turn : Math.sign(turn) * step;

      const speed = charging ? ELEPHANT.chargeSpeed : ELEPHANT.patrolSpeed;
    ...
    state.x = Math.max(-ELEPHANT.halfWidth, Math.min(ELEPHANT.halfWidth, state.x));
    state.z = Math.max(ELEPHANT.minZ, Math.min(ELEPHANT.maxZ, state.z));
  }

  /**
   * True when this player has been trampled.
   *
   * Evaluated on the server tick against the server's own elephant position,
   * so there is nothing for a client to dispute.
   */
  hits(state: ElephantState, player: PlayerState): boolean {
    if (!inElephantTerritory(player.z)) return false;
    const reach = ELEPHANT.radius + MOUNT_RADIUS;
    if (Math.abs(player.z - state.z) > reach) return false;
    if (Math.abs(player.x - state.x) > reach) return false;
    // Jumping does not save you - it is an elephant, not a rope.
    return player.y < 6;
  }
  ...
  private pickTarget(
    ...
    let bestDistance = ELEPHANT.aggroRange;

    for (const player of players) {
      if (!player.ready) continue;
      if (!inElephantTerritory(player.z)) continue;
      ...
      const range = player.sessionId === this.target ? ELEPHANT.leashRange : bestDistance;
